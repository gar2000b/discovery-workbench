...
/**
 * Detects mouse over, mouse or finger touch presses, and clicks on an
 * actor. A touch must go down over the actor and is considered
 * pressed as long as it is over the actor or within the
 * {@link #setTapSquareSize(float) tap square}. This behavior makes it
 * easier to press buttons on a touch interface when the initial touch
 * happens near the edge of the actor. Double clicks can be detected
 * using {@link #getTapCount()}. Any touch (not just the first) will
 * trigger this listener. While pressed, other touch downs are
 * ignored.
 * 
 * @author Nathan Sweet
 */
public class ClickListener extends InputListener {
	/**
	 * Time in seconds {@link #isVisualPressed()} reports true after a
	 * press resulting in a click is released.
	 */
	 ...
	 * {@link #clicked(InputEvent, float, float)} is only called for
	 * left clicks.
	 * 
	 * @see #ClickListener(int)
	 */
	public ClickListener(DeviceInputProcessor deviceInputProcessor, int button) {
	...
	public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
		...
		deviceInputProcessor.processTouchDown(event);
		return true;
	}
	...
	public void touchDragged(InputEvent event, float x, float y, int pointer) {
		...
		if (pressed && pointer == 0 && button != -1 && !Gdx.input.isButtonPressed(button))
			pressed = false;
		if (!pressed) {
			// Once outside the tap square, don't use the tap square
			// anymore.
			invalidateTapSquare();
		}
		deviceInputProcessor.processTouchDragged(event);
	}
	...
	public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
		...
		deviceInputProcessor.processTouchUp(event);
	}
	...
	/**
	 * Returns true if a touch is over the actor or within the tap
	 * square.
	 */
	...
	/**
	 * Returns true if a touch is over the actor or within the tap
	 * square or has been very recently. This allows the UI to show a
	 * press and release that was so fast it occurred within a single
	 * frame.
	 */
	 ...
	 *            time in seconds that must pass for two touch down/up
	...
	/**
	 * Returns the number of taps within the tap count interval for
	...
	/**
	 * The button that initially pressed this button or -1 if the
	...
	/**
	 * The pointer that initially pressed this button or -1 if the
	 * button is not pressed.
	 */
